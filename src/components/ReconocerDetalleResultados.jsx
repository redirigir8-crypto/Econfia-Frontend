  ...
  return (
    <div className="rounded-2xl border border-line/15 bg-surface-2/70 p-4 shadow-lg shadow-black/5 transition hover:border-brand/25 hover:bg-surface">
      <div className="mb-2 flex items-start justify-between gap-3">
        ...
        {badge && (
          <span className="shrink-0 rounded-md border border-brand/25 bg-brand/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-brand">
            {badge}
          </span>
        )}
      </div>
...
export default function ReconocerDetalleResultados({ consultaId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e.message || "No fue posible cargar Econfia Contact Search.");
      } finally {
  ...
  return (
    <div className="grid gap-4">
      ...
      {/* JSON crudo */}
      <div>
        <button type="button" onClick={() => setShowJson((v) => !v)} className="text-xs font-semibold text-brand hover:text-brand">
